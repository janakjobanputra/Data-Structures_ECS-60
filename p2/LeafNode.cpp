...
LeafNode* LeafNode::remove(int value)
{
  ...
  int borrow=0;
  //Checking whether leaf is less than half
  if(count < (leafSize+1)/2)
  {
    ...
    {
      borrow = leftSibling->getMaximum();
      //cout << borrow << endl;
      insert(borrow);
      leftSibling->remove(borrow);
      //pull from left, add to self, manipulate count
    }

    //BORROW RIGHT
    else if(rightSibling && rightSibling->getCount() > (leafSize+1)/2)
    {
      borrow = rightSibling->getMinimum();
      insert(borrow);
